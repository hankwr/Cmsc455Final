...
public class CTService {
	 ...
	 * @return the parsed Instant object, returns EPOCH if failed
	 */
	static public Instant strToInst(String time) {
		...
		try { // My intellisense freaks out about unreachable code without this tryCatch block
			status = time.matches("^\\d{1,2}:\\d{2}:\\d{2} [A-Z]{2}, \\d{1,2}/\\d{2}/\\d{4} [A-Z]{1,3}$");
	 ...
	 * @param timestamp the input String timestamp to check
	 * @return 
	 */
	static public boolean checkFormat(String timestamp) {
		
		boolean status;
		
		try { // Intellisense freaks out about unreachable code without this tryCatch block
			status = timestamp.matches("^\\d{1,2}:\\d{2}:\\d{2} [A-Z]{2}, \\d{1,2}/\\d{2}/\\d{4} [A-Z]{1,3}$");
		} catch (PatternSyntaxException e){
			e.printStackTrace();
			return false;
		}
		...
		try { // Use tryCatch block to verify that Instant is valid (Feb 30th would be invalid, for example)
			Instant.from(formatter.withResolverStyle(ResolverStyle.STRICT).parse(timestamp));
		} catch (DateTimeParseException e) {
			return false;
		} catch (Exception e) {
			e.printStackTrace();
